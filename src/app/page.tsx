...
const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  const sortedDatasets = useMemo(() => {
    const filtered = dashboardSearchTerm
      ? datasets.filter(ds => ds.name.toLowerCase().includes(dashboardSearchTerm.toLowerCase()))
      : datasets;
    ...
    <div className={`flex-1 overflow-y-auto p-8 ${theme.app} ${theme.text}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 h-full">
            <div className={`rounded-2xl p-6 shadow-sm border h-full ${theme.kpiCard} border-zinc-800`}>
                        ...
                        <thead className={`${theme.table}/50`}>
                            <tr>
                                {['Name', 'Type', 'Status', 'Records', 'Size', 'Last Updated'].map(header => (
                                    <th key={header} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-zinc-400">{header}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody className={`${theme.table.replace('border-[#126C86]/50','')} divide-y border-zinc-800`}>
                                ...
                                <tr key={ds.id} className="hover:bg-zinc-800/20 transition-colors">
                                    <td className="px-4 py-3 whitespace-nowrap">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                                <div className={`text-sm font-medium ${theme.tableCell}`}>{ds.name}</div>
                                                <div className={`text-xs ${theme.tableCellSubtle}`}>{ds.id}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className={`px-4 py-3 whitespace-nowrap text-sm ${theme.tableCellSubtle}`}>{ds.type}</td>
                                    <td className="px-4 py-3 whitespace-nowrap">
                                        <span className={`px-3 py-1 text-xs font-medium text-white rounded-full ${getStatusClass(ds.status)}`}>{ds.status}</span>
                                    </td>
                                    <td className={`px-4 py-3 whitespace-nowrap text-sm ${theme.tableCellSubtle}`}>{ds.records}</td>
                                    <td className={`px-4 py-3 whitespace-nowrap text-sm ${theme.tableCellSubtle}`}>{ds.size}</td>
                                    <td className={`px-4 py-3 whitespace-nowrap text-sm ${theme.tableCellSubtle}`}>{formatDate(ds.lastUpdate)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                ...
                <div className="flex justify-start items-center mt-4">
                  <button onClick={() => setActivePage(Page.DATASETS)} className="text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg px-4 py-2 transition-colors shadow-sm">View Full Datasets</button>
                </div>
            </div>
        </div>
